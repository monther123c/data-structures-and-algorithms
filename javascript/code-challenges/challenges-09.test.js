...
const checkValues = (obj, value) => {
  return Object.values(obj) == value ? true : false
  // Solution code here...
};
...
const getHouses = (arr) => {
  let houses = [];
  arr.forEach((item) => houses.push(item.house));
  // Solution code here...
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  const newArr = arr.filter(item => Object.values(item).length > 3);
	if(newArr.filter(item => item.name === character).length > 0){
		return true
	} else {
		return false
	}

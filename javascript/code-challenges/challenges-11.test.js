...
function transformToLis(obj){
  ...
  let transformArr = [];
  data.map(item => {
    transformArr.push(`<li>${item[0]}: ${item[1]}</li>`);
  });
  return transformArr;
};
  ...
  input.map((element, index) => {
    ...
    element.map(element2 => {
      if (typeof (element2) === 'number' && element2 % 5 == 0) {
        let newvalue = Math.pow(2, element2);
        divisible[index].push(newvalue);
      }
    });
  });
  return divisible;
...
let findMaleAndFemale = (data) => {
  ...
  data.map(item => {
    if (item.gender == 'female' || item.gender == 'male') {
      geneder.push(item.name);
    }
  });
  let maleNfemale = geneder.join(' and ');
  return maleNfemale;
  // Solution code here...
};
...
let findShortest = (data) => {
  let shortest = data.sort((first, second) => {
    if (parseInt(first.height) < parseInt(second.height)) {
      return -1;
    }
  });
